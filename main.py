...
from nnkit import Sigmoid, Softmax, DenseLayer, DenseNetwork, CrossEntropySoftmax, ReLU, UpdateRule, Optimizer, SGD
...
def compute_gradient(net, loss, X, T) -> np.ndarray:
    ...
    for x, t in zip(X, T):
        gradient += net.backprop(loss, x, t)
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        with Pool(processors) as pool:
            backprop_arguments = [(net, loss, train_images_chunks[i], train_labels_hot_chunks[i]) for i in
                                  range(0, processors)]
